...
const AREAS = [
  { nombre: "Psicológica", desc: "Estrés, ansiedad, conflictos laborales.", icon: "🧠" },
  { nombre: "Psiquiátrica", desc: "Evaluación clínica, medicación.", icon: "💊" },
  { nombre: "Traumatológica", desc: "Lesiones laborales, ergonomía.", icon: "🦴" },
...
export default function AsesoramientoIntegral() {
  ...
  // --------------------------------
  // Autocompletar datos reales ASMEL
  // --------------------------------
  const usarMisDatos = () => {
    if (!user) return;
    ...
    setEmail(user.contacto?.email || "");
    setCelular(user.contacto?.telefono || "");
  };
  ...
  const validarFormulario = () => {
    ...
    if (!area) errs.area = "Seleccioná un área";
    if (area === "Otro" && otroMotivo.trim().length < 5)
      errs.otroMotivo = "Describí el motivo";
    if (!empresa.trim()) errs.empresa = "Ingresá la empresa";
    if (!/^\d{11}$/.test(cuit)) errs.cuit = "CUIT inválido";
    if (!/^[A-Za-zÁÉÍÓÚáéíóúÑñ\s]{3,}$/.test(nombre)) errs.nombre = "Nombre inválido";
    if (!/^\d{8,14}$/.test(celular)) errs.celular = "Celular inválido";
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) errs.email = "Email inválido";
    if (consulta.trim().length < 10) errs.consulta = "Consulta demasiado breve";
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const payload = {
      ...
      area,
      descripcion: area === "Otro" ? otroMotivo.trim() : consulta.trim(),
    ...
    try {
      const res = await fetch(`${API_URL}/api/asesoramiento/solicitar`, {
        method: "POST",
        credentials: "include",
      ...
      setMsg({
        type: "ok",
        text: "Solicitud enviada correctamente. Te contactaremos.",
      });
  ...
  const msgWhatsApp = encodeURIComponent(
    `Hola, necesito asesoramiento en el área: ${area || "Sin especificar"}.
Empresa: ${empresa || "-"}
CUIT: ${cuit || "-"}
Contacto: ${nombre || "-"} - ${celular || "-"}`
  ...
  return (
    <div className="max-w-5xl mx-auto bg-white/10 backdrop-blur-lg p-8 rounded-3xl shadow-xl text-white">
      ...
      <h3 className="text-xl font-semibold mb-3">Seleccioná el área de asesoramiento</h3>
      ...
      <div className="mt-4 text-center">
        <a
          href={`https://wa.me/5491122334455?text=${msgWhatsApp}`}
          target="_blank"
