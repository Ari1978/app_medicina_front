...
export default function TurnoForm() {
  ...
  const [horariosDisponibles, setHorariosDisponibles] =
    useState<HorarioDisponible[]>([]);

  // map de examenes por puesto
  ...
  // ============================================================
  // 🔥 CARGA DE CUPOS (confirmados + provisionales del carrito)
  // ============================================================
  useEffect(() => {
    if (!fecha || !user) {
    ...
    const cargarHorarios = async () => {
      try {
        ...
        // Provisionales en carrito del user
        const resCarrito = await fetch(`${API_URL}/api/carrito`, {
          credentials: "include",
        });
        const provisionales = resCarrito.ok ? await resCarrito.json() : [];

        const mapProv: Record<string, number> = {};
        provisionales.forEach((t: any) => {
          if (t.fecha === isoFecha) {
        ...
        // Armar horarios
        const horarios = FRANJAS.map((hour) => {
          const usadosConfirm = mapConfirm[hour] || 0;
          const usadosProv = mapProv[hour] || 0;
          const libres =
            MAX_TURNOS_POR_HORA - usadosConfirm - usadosProv;

          return {
            hora: hour,
            disponibles: libres > 0 ? libres : 0,
          };
        });
  ...
  // ============================================================
  // 🔥 SUBMIT → crear turno provisional → al carrito
  // ============================================================
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!fecha || !horaSeleccionada)
      return alert("Debe seleccionar fecha y hora");
    ...
    // El owner real del turno es el usuario empresa
    ...
    const payload = {
      ...
      provisional: true,
      user: ownerId, // backend setea empresaId y trazabilidad
    };
    ...
    try {
      ...
      console.error("❌ No se pudo agregar el turno", err);
  ...
  return (
    <main className="top-10 flex flex-col items-center w-full min-h-screen py-10 relative">
      ...
      {/* Carrito flotante */}
      <div
        className="fixed top-28 right-5 z-50 cursor-pointer"
